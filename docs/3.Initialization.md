...
2. Provisioning
    ...
    - As mentioned above, at this step the **[Mobile SDK Wallet](https://gitlab.com/evernym/mobile/mobile-sdk/-/blob/main/docs/2.Storage.md#information-stored-by-mobile-sdk)** will be initialized. You will not need any additional configuration or steps other than providing a key for the wallet. `wallet_key` inside `vcx_config` in the above code sample is the key the client's app needs to provide to initialize/encrypt/decrypt the wallet.
    The generation of this key depends on the client app. Here is an example implementation of the key generation [on Android](https://gitlab.com/evernym/mobile/mobile-sdk/-/blob/main/examples/android/MSDKSampleAppJava/lib/src/main/java/msdk/java/types/ProvisioningConfig.java#L120) and [iOS](https://gitlab.com/evernym/mobile/mobile-sdk/-/blob/main/examples/ios/common/Config.m#L42).  
