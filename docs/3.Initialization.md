...
2. Provisioning
    ...
    - As mentioned above, at this step, the **[Mobile SDK Wallet](https://gitlab.com/evernym/mobile/mobile-sdk/-/blob/main/docs/2.Storage.md#information-stored-by-mobile-sdk)** will be initialized. The wallet needs a key, and `wallet_key` inside `vcx_config` in the above code sample is the key the client's app needs to provide to initialize/encrypt/decrypt the wallet.
    The generation of this key depends on the client app. You can find an example of how key generation can be implemented [here (Java)](https://gitlab.com/evernym/mobile/mobile-sdk/-/blob/main/examples/android/MSDKSampleAppJava/lib/src/main/java/msdk/java/types/ProvisioningConfig.java#L120) and [here (Swift and Objective-C)](https://gitlab.com/evernym/mobile/mobile-sdk/-/blob/main/examples/ios/common/Config.m#L42). 
